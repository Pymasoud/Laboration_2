...
class Deck:
    ...
    def insert(card_list, card):
        ...
        # If the list is empty, just add the first card to it
        if len(card_list) == 0:
            card_list.append(card)

        # If the card is bigger than our last card add it to the end of the list
        elif card > card_list[-1]:
            card_list.append(card)

        else:
            # Creating a counter named "index"
            index = 0
            while len(card_list) < 52:
                # If the cards have the same or smaller rank , add it to the list at that index
                if card < card_list[index] or card == card_list[index]:
                    card_list.insert(index, card)
                    break

                # If our card is bigger than the card at the current index, increase index
                elif card > card_list[index]:
                    index += 1
